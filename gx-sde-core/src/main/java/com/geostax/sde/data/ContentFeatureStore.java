 ...
 * <p>
 * List its base class {@link ContentFeatureSource}, this feature store works
 * off of operations provided by {@link FeatureCollection}.
 * </p>
 * <p>
 * The {@link #addFeatures(SimpleFeatureCollection)} method is used to add
 * features to the feature store. The method should return the "persistent"
 * feature id's which are generated after the feature has been added to
 * persistent storage. Often the persistent fid is different from the fid
 * specified by the actual feature being inserted. For this reason
 * {@link SimpleFeature#getUserData()} is used to report back persistent fids.
 * It is up to the implementor of the feature collection to report this value
 * back after a feature has been inserted. As an example, consider an
 * implementation of {@link FeatureCollection#add(Object)}.
 * 
 * <pre>
...
public abstract class ContentFeatureStore extends ContentFeatureSource
	 ...
	 * 
	 * @param entry
	 *            The entry for the feature store.
	 * @param query
	 *            The defining query.
	 */
	 ...
	 *            Query
	 * @param flags
	 *            See {@link #WRITER_ADD} and {@link #WRITER_UPDATE}
	 */
	...
	FeatureId addFeature(final SimpleFeature feature, FeatureWriter<SimpleFeatureType, SimpleFeature> writer)
		...
		// Need to save a link to the original feature in order to be able to
		// set the ID once it
		...
		// copy any metadata from the feature that was actually written (not
		// always effective, see
	 ...
	 * <p>
	 * This method operates by first clearing the contents of the feature store
	 * ({@link #removeFeatures(Filter)}), and then obtaining an appending
	...
	public final void setFeatures(FeatureReader<SimpleFeatureType, SimpleFeature> reader) throws IOException {
		...
		try {
			while (reader.hasNext()) {
				...
				// grab next feature and populate it
				// JD: worth a note on how we do this... we take a "pull"
				// approach
				// because the raw schema we are inserting into may not match
				// the
				// schema of the features we are inserting
	...
	/**
	 * Modifies/updates the features of the store which match the specified
	 * filter.
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * The <tt>filter</tt> must not be <code>null</code>, in this case this
	...
	final public void modifyFeatures(String name, Object attributeValue, Filter filter) throws IOException {
	 ...
	 * <p>
	 * This method operates by obtaining an updating feature writer based on the
	 * specified <tt>filter</tt> and removing every feature from it.
	 * </p>
	 * <p>
	 * The <tt>filter</tt> must not be <code>null</code>, in this case this
	 * method will throw an {@link IllegalArgumentException}.
